...
class RobotCommanderInterface(Node):
    ...
    def moveTray(self):
        if self.tray:
            self.get_logger().info("👉 Inside Tray code...")
    ...
    def movePart(self):
        # self.get_logger().info("Inside Move Part...")
        if self.part:
            self.get_logger().info("👉 Inside Part code...")
            ...
            # move to tray
            if self._activated_gripper:
                # self.get_logger().info("activated gripper line 288 ..................")
                # Move the robot to the tray
                # TODO: Check whether the tray is picked up by using
                # a subscriber to /ariac/floor_robot_gripper_state
                if not self._moving_robot_to_part:
                    # TODO: get the tray id and pose from the vision system
                    part = Part()
                    ...
                    self.get_logger().info("Calling move robot to part")
                    self._move_robot_to_part(part, part_pose)
                
            if self._moved_robot_to_part:
                self.get_logger().info(" _moved_robot_to_part line 311 ..................")
                # TODO: Check whether the tray is attached to the gripper before proceeding
                if not self._moving_part_to_agv:
                    self.get_logger().info(" _moving_part_to_agv line 314 ..................")
                    self._move_part_to_agv(PlacePartOnTray.Request.AGV1,PlacePartOnTray.Request.QUADRANT1)
            
            # Deactivate gripper
            # TODO: Check whether the tray is not attached to the gripper
            # by using a subscriber to /ariac/floor_robot_gripper_state
            if self._moved_part_to_agv:
                if not self._deactivating_gripper:
                    self._deactivate_gripper()
    ...
    def _move_robot_to_part(self, part, part_pose):
        """
        Move the floor robot to a part to pick it up
        """
        self.get_logger().info("👉 Moving robot to part...")
        self._moving_robot_to_part = True
        ...
        future = self._move_robot_to_part_cli.call(request)
    ...
    def _move_part_to_agv(self, agv_number, quadrant):
        ...
        self.get_logger().info("👉 Moving part to AGV...")
        self._moving_part_to_agv = True

        while not self._move_part_to_agv_cli.wait_for_service(timeout_sec=1.0):
            self.get_logger().info("Service not available, waiting...")

        request = PlacePartOnTray.Request()
        request.agv_number = agv_number
        request.quadrant = quadrant
        future = self._move_part_to_agv_cli.call(request)
    #     future.add_done_callback(self._move_part_to_agv_done_cb)

    # def _move_part_to_agv_done_cb(self, future):
    #     """
    #     Client callback for the service /commander/place_part_on_tray
