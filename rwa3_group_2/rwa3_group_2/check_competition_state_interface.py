...
class CheckCompetitionStateInterface(Node):
    ...
    def end_competition(self):
        ...
        # Check if competition state is ended, if not ready spin the node.
        while self._competition_state != CompetitionState.ENDED:
            try:
                rclpy.spin_once(self)
            except KeyboardInterrupt:
                self.get_logger().error('KeyboardInterrupt received!')
        # Check if the competition state is ready, if ready send the request to service.
        if not self._low_orders and not self._high_orders and self._competition_state == CompetitionState.ORDER_ANNOUNCEMENTS_DONE:
            # Creating Request
            request = Trigger.Request()
            # Async call to the service.
