...
class ImageSubscriber(Node):
            ...
            if len(msg._part_poses) == 0:
                ...
                if self._right_bin_parts != len(msg._part_poses):
                    for i, part_pose in enumerate(msg._part_poses):
                        self.get_logger().info(f'DETECTED part in Right bin {i+1}: ')
                                ...
                                if color == colorr and comp == component:
                                    ...
                                    if hasattr(self, attr_name):
                                        ...
                                        if ort_var not in target_list:  
                                            ...
                                            # self.get_logger().info(f"Updated {self._dictionary}")  

                    self.get_logger().info(f"Added parts from RIGHT camera {self._dictionary}")  
        ...
        try:
            if len(msg._part_poses) == 0:
                ...
                if self._left_bin_parts != len(msg._part_poses):
                    for i, part_pose in enumerate(msg._part_poses):
                        self.get_logger().info(f'DETECTED part in LEFT bin {i+1}: ')
                        color=part_pose.part.color
                        comp=part_pose.part.type
                    ...
                    self.get_logger().info(f"Added parts from the LEFT camera: {self._dictionary}") 
    ...
    def _kts1_camera_cb(self,msg):
        try:
            if len(msg.tray_poses) == 0:
                ...
                if self._kts1_trays != len(msg.tray_poses):
                    ...
                    for i, tray_pose in enumerate(msg.tray_poses):
                        self.get_logger().info(f'DETECTED KTS1:----- {i+1}: ')
                        I_D=tray_pose.id
                        ...
                        # oY=round(trans_coords[4],2)
                        oZ=oZ=3.14 if round(trans_coords[5],2) == -3.14 else round(trans_coords[5],2)
                        self.get_logger().info(f'Detected KTS1 ------Tray{i+1} ID :{I_D},')
                        # self.get_logger().info(f'Part {i+1} Location: X :{X}, Y :{Y}, Z :{Z}, oX :{oX}, oY :{oY}, oZ :{oZ} ')
                        ort_var=(X,Y,Z,oX,oY,oZ)
                        ...
                        if hasattr(self, attr_name):
                            ...
                            if ort_var not in target_list:  
                                ...
                                # self.get_logger().info(f"Added tray ID {attr_name}: {ort_var} by KTS1")
                    self.get_logger().info(f'Added trays from the KTS1 camera :: {self._tray_dict}')
            ...
            self.get_logger().error('KTS1 ALC failed to process the part poses: %r' % (e,))
        ...
        try:
            if len(msg.tray_poses) == 0:
                ...
                if self._kts2_trays != len(msg.tray_poses):
                    ...
                    for i, tray_pose in enumerate(msg.tray_poses):
                        self.get_logger().info(f'DETECTED KTS2 tray {i+1}: ')
                        I_D=tray_pose.id
                        ...
                        # oY=round(trans_coords[4],2)
                        oZ=oZ=3.14 if round(trans_coords[5],2) == -3.14 else round(trans_coords[5],2)
                        self.get_logger().info(f'Detected KTS2 Tray{i+1} with---- ID :{I_D},')
                        # self.get_logger().info(f'Part {i+1} Location: X :{X}, Y :{Y}, Z :{Z}, oX :{oX}, oY :{oY}, oZ :{oZ} ')
                        ort_var=(X,Y,Z,oX,oY,oZ)
                        ...
                        if hasattr(self, attr_name):
                            ...
                            if ort_var not in target_list:  
                                target_list.add(ort_var)
                                self._tray_dict.update({attr_name:target_list})
                                self.get_logger().info(f"Added Trays ID {attr_name}: {ort_var} by KTS2")
                    self.get_logger().info(f"Added Trays from the KTS2 Camera:  {self._tray_dict}") 
            ...
            # print(traceback.format_exc())
            self.get_logger().error(' BLC failed to process the part poses: %r' % (e,))
